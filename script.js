...
textInput.addEventListener('click', (e)=>{
	positionCursor = textInput.selectionStart;		
});
...
wrapper.addEventListener('click', (e)=>{
	...
	let b = 'button-small ';
	if(a!='key-wrapper' && a!='line' && a!=b+'tab' && a!=b+'bigBtnL' && a!=b+'bigBtnR' && a!=b+'littleBtn' && a!=b+'space' && a!=b+'bigBtnL active' && a!=b+'littleBtn active'  && a!=b+'bigBtnR active'   && a!=b+'tab active'){
	...
	textInput.click()
		...
		if(!checkBack){
		textInput.click()
		checkPosotion = positionCursor; 
		}
		checkPosotion = positionCursor; 
		if(checkPosotion!=0)
		start = textInput.value.slice(0,positionCursor-1);
		let end = textInput.value.slice(positionCursor,textInput.value.length);
		textInput.value='';
		...
		checkPosotion = positionCursor; 
		if(checkPosotion!=0)
		start = textInput.value.slice(0,positionCursor);
		let end = textInput.value.slice(positionCursor+1,textInput.value.length);
		textInput.value='';
		textInput.value=start;
		textInput.click()
		checkBack=1;
...
wrapper.addEventListener('mousedown', (e)=>{
	...
	if(a!='key-wrapper' && a!='line'){
		...
		if(e.target.innerText!='CapsLock'){
			let interval = setInterval(() => {radius=+radius; if(radius<20){radius+=1;e.target.style.borderRadius=`${radius}px`;}else return;}, 5);
...
wrapper.addEventListener('mouseup', (e)=>{
	...
	let radius =20;
	let interval = setInterval(() => {radius=+radius; if(radius>0){radius-=1;e.target.style.borderRadius=`${radius}px`;}else return;}, 5);
